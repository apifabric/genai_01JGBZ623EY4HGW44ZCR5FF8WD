...
resourcename="restaurantmanagement"  # lower case, only
resourcegroup="restaurantmanagement_rg"
...
# git clone https://github.com/apilogicserver/restaurantmanagement.git
...
az group create --name restaurantmanagement_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group restaurantmanagement_rg --sku S1 --is-linux
az container create --resource-group restaurantmanagement_rg --name restaurantmanagement --image apilogicserver/restaurantmanagement:latest --dns-name-label restaurantmanagement --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//restaurantmanagement.westus.azurecontainer.io:5656'
